...
from context_manager_templates.cmt_linear import ExtendedMessage, CMTLinear
from context_manager_templates.cmt_linear import find_sublist_indices, replace_token_ids
...
class LinearThinkCMT(CMTLinear):
    ...
    def group_tokenize_turns(self):
        ...
        for turn_idx in range(max_turn):
            ...
            if turn_idx < len(self.grouped_steps):
                ...
                ext_steps = self.grouped_steps[-1]
                cmt_tokenized = self.tokenize_steps_pure_padding(ext_steps=ext_steps)

            sample = Sample(
                data_id=self.data_id,
                rollout_id=self.rollout_id,
                task_id=self.task_id,
                minor_index_id=turn_idx,
                messages=self.to_role_content(ext_steps),
                input_ids=cmt_tokenized["input_ids"],
                prompt_ids=cmt_tokenized["prompt_ids"],
                response_ids=cmt_tokenized["response_ids"],
    ...
    def tokenize_steps_pure_padding(self, ext_steps: List[ExtendedMessage]) -> dict:
        """
        ...
        Note:
            ...
            - In this version, all loss_masks are set to 0 to ensure the policy is not updated.
        """
        ...
        for ext_msg in ext_steps:
            ...
            # 关键修改：将所有的loss_mask设置为0，确保不进行策略更新
            loss_mask += [0] * len(ext_msg.token_arr)
