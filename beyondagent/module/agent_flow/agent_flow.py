...
from context_manager_templates.cmt_linear import CMTLinear, ExtendedMessage
from context_manager_templates.cmt_memory import MemoryCMT, GroupedSteps
from context_manager_templates.cmt_linear_think import LinearThinkCMT
from context_manager_templates.cmt_context_aware import SelfContextAwareCMT
from context_manager_templates.cmt_context_clip import SelfContextClipCMT
...
class AgentFlow(BaseAgentFlow):
    ...
    def __init__(self, **kwargs):
        ...
        self.cmt: Union[CMTLinear, MemoryCMT, LinearThinkCMT, SelfContextAwareCMT, None] = None
        self.console_debug_mode: bool = self.config.actor_rollout_ref.rollout.debug_llm_io

    def add_experience(self, init_messages, task_id, data_id, rollout_id, query):
        if self.config.experience_maker.enable_context_generator:
            ...
            if history_experience:
                ...
                if "context_time_cost" in trajectory.metadata:
                    self.cmt.context_time_cost = trajectory.metadata["context_time_cost"]
                init_messages["content"] = new_content
            else:
    ...
    def execute(self, init_messages: List[dict], env: EnvClient, instance_id: str, tmux, stop, thread_index, task_id, data_id="", rollout_id="", query="", **kwargs) -> CMTLinear:

        # 1. 🚀 Initialize messages
        if self.config.actor_rollout_ref.rollout.context_template == "summary":
            self.cmt: MemoryCMT = MemoryCMT(self.config, self.tokenizer)
        elif self.config.actor_rollout_ref.rollout.context_template == "linear":
            self.cmt: CMTLinear = CMTLinear(self.config, self.tokenizer)
        elif self.config.actor_rollout_ref.rollout.context_template == "linear_think":
            self.cmt: LinearThinkCMT = LinearThinkCMT(self.config, self.tokenizer)
        elif self.config.actor_rollout_ref.rollout.context_template == "context_selfaware":
            self.cmt: SelfContextAwareCMT = SelfContextAwareCMT(self.config, self.tokenizer, self.llm_chat_fn)
        elif self.config.actor_rollout_ref.rollout.context_template == "context_selfclip":
            self.cmt: SelfContextClipCMT = SelfContextClipCMT(self.config, self.tokenizer, self.llm_chat_fn)
        else:
        ...
        init_messages = self.add_experience(init_messages, task_id, data_id, rollout_id, query)
        ...
        for act_step in range(self.max_steps):
            ...
            try:
                ...
                env_output["state"] = env_output["state"][0] if isinstance(env_output['state'], list) else env_output["state"]
            ...
            self.cmt.is_terminated = env_output["is_terminated"] if 'is_terminated' in env_output else env_output['data']["is_terminated"]
        ...
        with log_generate_lock:
            ...
            time.sleep(0.05)  # Ensure log is written before proceeding
